...
\begin{itemize}
	...
	\setlength{\itemsep}{0pt}
	\setstretch{1.05}
	\item Jako první upravíme pom.xml (Kod.~\ref{uprava-pom-szdc-position.xml}) soubor do kterého doplníme novou část kodu v sekci <plugin>. Pomocí tohoto přidaného kódu pak při opětovném sestavení projektu pomocí mavenu dojde k autogenerování tříd datového rozhraní GRAPP, které pak využijem pro načítání dat.
	...
	\item Následuje tvorba interface provideru (Kod.~\ref{SzdcProvider.java}) a komunikačních přepravek (Obr.~\ref{fig:szdc-position-models}).
 
 	\obr{szdc-position-models}{fig:szdc-position-models}{1.0}{graphics/szdc-position-models.jpg}
...
Další možností načítaní zdroje dat o spoždění je datové rozhraní informační tabule. Při implementaci jsem nakonec toto datové rozhraní pro načítaní spoždění nepoužily. Toto rozhraní je primárně  využito k načítaní informaci o zastávkách a nástupištích kudy spoj projíždí. Proto se přistoupilo k napojení na sluzbu polohavozu.cz.

\n{2}{Připojení Polohavozu.cz}
V první fázi byla napojena datova rozhraní od SŽDC, bohužel tyto data neobsahovala všechna potřebná data. Chybjejícím prvkem byla souřadnice GPS potřebná k určení kde se vůz se spožděním nachází.
\begin{itemize}
