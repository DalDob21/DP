...
Jako první upravíme pom.xml (Kod.~\ref{uprava-pom-szdc-position.xml}) soubor do kterého přidáme v sekci <plugin> nové údaje. Pomocí tohoto přidaného kódu pak při opětovném sestavení projektu pomocí mavenu dojde k autogenerování tříd, které pak využijem pro načítání dat.
 
\script{uprava-pom-szdc-position.xml}{xml}
 
Dalším krokem je tvorba service , přes kterho se budem dotazovat na datove rozhrani.
 
\script{SzdcPositionWSApiService.java}{Java}
 
Následuje tvorba interface provideru a komunikacnich prepravek.
 
\script{SzdcProvider.java}{Java}
 
Implemntace provideru a jeho metod, které budem využívat.
 
\script{SzdcProviderImpl.java}{Java}
 
Pokud mame komunikační třídu provideru implentovanu, bude potřeba vytvořit 
